<!-- Autogenerated from VectorSource. Do not modify. -->
## <MapboxGL.VectorSource />
### VectorSource is a map content source that supplies tiled vector data in Mapbox Vector Tile format to be shown<br/>on the map. The location of and metadata about the tiles are defined either by an option dictionary or by an<br/>external file that conforms to the TileJSON specification.

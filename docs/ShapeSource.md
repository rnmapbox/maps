...
| clusterProperties | `object` | `none` | `false` | Specifies custom properties on the generated clusters if clustering<br/>is enabled, aggregating values from clustered points.<br/><br/>Has the form `{ "property_name": [operator, map_expression]}`, where<br/> `operator` is any expression function that accepts at least 2 operands (e.g. `"+"` or `"max"`) -<br/>  it accumulates the property value from clusters/points the cluster contains<br/> `map_expression` produces the value of a single point<br/>  Example: `{ "sum": ["+", ["get", "scalerank"]] }`<br/><br/>Example: `{ "sum": ["+", ["get", "scalerank"]] }`<br/>For more advanced use cases, in place of `operator`, you can use a custom reduce expression<br/>that references a special `["accumulated"]` value, e.g.:<br/>`{ "sum": [["+", ["accumulated"], ["get", "sum"]], ["get", "scalerank"]] }` |

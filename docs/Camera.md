...
# <MapboxGL.Camera />
Controls the perspective from which the user sees the map.<br/><br/>To use imperative methods, pass in a ref object:<br/><br/>```typescript<br/>const camera = useRef<Camera>(null);<br/><br/>useEffect(() => {<br/>  camera.current?.setCamera({<br/>    centerCoordinate: [lon, lat],<br/>  });<br/>}, []);<br/><br/>return (<br/>  <Camera ref={camera} /><br/>);<br/>```

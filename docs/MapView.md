...
| localizeLabels | `shape` | `none` | `false` | [`mapbox` (v10) implementation only]<br/>Set map's label locale, e.g. { "locale": "es" } will localize labels to Spanish, { "locale": "current" } will localize labels to system locale. |
| &nbsp;&nbsp;locale | `string` | `none` | `true` | locale code like `es` or `current` for the device's current locale |
| &nbsp;&nbsp;layerIds | `Array` | `none` | `false` | layer id to localize. If not specified, all layers will be localized |
...
| onCameraChanged | `func` | `none` | `false` | iOS, v10 only, deprecated will be removed in next version - please use onRegionIsChanging.<br/>*signature:*`(state:{properties: {center: GeoJSON.Position, bounds: {ne: GeoJSON.Position, sw: GeoJSON.Position}, zoom: number, heading: number, pitch: number}, gestures: {isGestureActive: boolean, isAnimatingFromGesture: boolean}}) => void` |
| onMapIdle | `func` | `none` | `false` | iOS, v10 only, deprecated will be removed in next version - please use onRegionDidChange<br/>*signature:*`(state:{properties: {center: GeoJSON.Position, bounds: {ne: GeoJSON.Position, sw: GeoJSON.Position}, zoom: number, heading: number, pitch: number}, gestures: {isGestureActive: boolean, isAnimatingFromGesture: boolean}}) => void` |

...
- (void)updateProps:(const Props::Shared &)props oldProps:(const Props::Shared &)oldProps
{
  const auto &newProps = *std::static_pointer_cast<const MBXSymbolLayerProps>(props);
    ...
    _view.filter = [MBXSymbolLayerComponentView convertDynamicArrayToArray:&newProps.filter];
    ...
    _view.reactStyle = [MBXSymbolLayerComponentView convertDynamicToDictionary:&newProps.reactStyle];

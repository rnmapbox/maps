...
- (void)updateProps:(const Props::Shared &)props oldProps:(const Props::Shared &)oldProps
{
  const auto &newProps = *std::static_pointer_cast<const MBXSkyLayerProps>(props);
    ...
    _view.filter = [MBXSkyLayerComponentView convertDynamicArrayToArray:&newProps.filter];
    ...
    _view.reactStyle = [MBXSkyLayerComponentView convertDynamicToDictionary:&newProps.reactStyle];

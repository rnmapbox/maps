...
@objc(MBXLayer)
public class MBXLayer : UIView, RCTMGLMapComponent, RCTMGLSourceConsumer {
  ...
  deinit {
    if let waitingForID = waitingForID {
      Logger.log(level:.warn, message: "MBXLayer.removeFromMap - unmetPositionDependency: layer: \(optional: id) was waiting for layer: \(optional: waitingForID) but it hasn't added to map")
  ...
  private func removeFromMap(_ style: Style) {
    ...
    do {
      ...
      Logger.log(level: .error, message: "MBXLayer.removeFromMap: removing layer failed for layer \(optional: id): \(error.localizedDescription)")
    }
  }

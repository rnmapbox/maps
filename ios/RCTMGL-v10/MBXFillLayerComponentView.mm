...
#import "MBXFillLayerComponentView.h"
...
@interface MBXFillLayerComponentView () <RCTMBXFillLayerViewProtocol>
@end

@implementation MBXFillLayerComponentView {
    MBXFillLayer *_view;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const MBXFillLayerProps>();
    _props = defaultProps;
    _view = [[MBXFillLayer alloc] init];
...
{
  return concreteComponentDescriptorProvider<MBXFillLayerComponentDescriptor>();
}
...
{
  const auto &newProps = *std::static_pointer_cast<const MBXFillLayerProps>(props);
    _view.id = RCTNSStringFromStringNilIfEmpty(newProps.id);
    _view.sourceID = RCTNSStringFromStringNilIfEmpty(newProps.sourceID);
    _view.filter = [MBXFillLayerComponentView convertDynamicArrayToArray:&newProps.filter];
    ...
    _view.reactStyle = [MBXFillLayerComponentView convertDynamicToDictionary:&newProps.reactStyle];
...
Class<RCTComponentViewProtocol> MBXFillLayerCls(void)
{
  return MBXFillLayerComponentView.class;
}

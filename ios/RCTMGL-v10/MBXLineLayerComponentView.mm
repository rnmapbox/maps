...
#import "MBXLineLayerComponentView.h"
...
@interface MBXLineLayerComponentView () <RCTMBXLineLayerViewProtocol>
@end

@implementation MBXLineLayerComponentView {
    MBXLineLayer *_view;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const MBXLineLayerProps>();
    _props = defaultProps;
    _view = [[MBXLineLayer alloc] init];
...
{
  return concreteComponentDescriptorProvider<MBXLineLayerComponentDescriptor>();
}
...
{
  const auto &newProps = *std::static_pointer_cast<const MBXLineLayerProps>(props);
    _view.id = RCTNSStringFromStringNilIfEmpty(newProps.id);
    _view.sourceID = RCTNSStringFromStringNilIfEmpty(newProps.sourceID);
    _view.filter = [MBXLineLayerComponentView convertDynamicArrayToArray:&newProps.filter];
    ...
    _view.reactStyle = [MBXLineLayerComponentView convertDynamicToDictionary:&newProps.reactStyle];
...
Class<RCTComponentViewProtocol> MBXLineLayerCls(void)
{
  return MBXLineLayerComponentView.class;
}

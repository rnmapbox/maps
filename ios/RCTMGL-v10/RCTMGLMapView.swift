...
open class RCTMGLMapView: MapView {
  var tapDelegate: IgnoreRCTMGLMakerViewGestureDelegate?

  var compassEnabled = false
  var compassFadeWhenNorth = false
...
extension RCTMGLMapView {
  ...
  @objc func setReactOnMapChange(_ value: @escaping RCTBubblingEventBlock) {
    ...
    self.onEvery(event: .cameraChanged, handler: { (self, _) in
    ...
    self.onEvery(event: .mapIdle, handler: { (self, _) in
...
extension RCTMGLMapView {
  func setSourceVisibility(_ visible: Bool, sourceId: String, sourceLayerId: String?) {
...
class PointAnnotationManager: AnnotationInteractionDelegate {
  weak var selected: RCTMGLPointAnnotation?

...
#import "MBXRasterLayerComponentView.h"
...
@interface MBXRasterLayerComponentView () <RCTMBXRasterLayerViewProtocol>
@end

@implementation MBXRasterLayerComponentView {
    MBXRasterLayer *_view;
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const MBXRasterLayerProps>();
    _props = defaultProps;
    _view = [[MBXRasterLayer alloc] init];
...
{
  return concreteComponentDescriptorProvider<MBXRasterLayerComponentDescriptor>();
}
...
{
  const auto &newProps = *std::static_pointer_cast<const MBXRasterLayerProps>(props);
    _view.id = RCTNSStringFromStringNilIfEmpty(newProps.id);
    _view.sourceID = RCTNSStringFromStringNilIfEmpty(newProps.sourceID);
    _view.filter = [MBXRasterLayerComponentView convertDynamicArrayToArray:&newProps.filter];
    ...
    _view.reactStyle = [MBXRasterLayerComponentView convertDynamicToDictionary:&newProps.reactStyle];
...
Class<RCTComponentViewProtocol> MBXRasterLayerCls(void)
{
  return MBXRasterLayerComponentView.class;
}

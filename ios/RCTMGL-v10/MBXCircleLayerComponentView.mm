...
#import "MBXCircleLayerComponentView.h"
...
@interface MBXCircleLayerComponentView () <RCTMBXCircleLayerViewProtocol>
@end

@implementation MBXCircleLayerComponentView {
    MBXCircleLayer *_view;
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const MBXCircleLayerProps>();
    _props = defaultProps;
    _view = [[MBXCircleLayer alloc] init];
...
{
  return concreteComponentDescriptorProvider<MBXCircleLayerComponentDescriptor>();
}
...
{
  const auto &newProps = *std::static_pointer_cast<const MBXCircleLayerProps>(props);
    _view.id = RCTNSStringFromStringNilIfEmpty(newProps.id);
    _view.sourceID = RCTNSStringFromStringNilIfEmpty(newProps.sourceID);
    _view.filter = [MBXCircleLayerComponentView convertDynamicArrayToArray:&newProps.filter];
    ...
    _view.reactStyle = [MBXCircleLayerComponentView convertDynamicToDictionary:&newProps.reactStyle];
...
Class<RCTComponentViewProtocol> MBXCircleLayerCls(void)
{
  return MBXCircleLayerComponentView.class;
}

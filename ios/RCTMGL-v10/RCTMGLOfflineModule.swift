...
class RCTMGLOfflineModule: RCTEventEmitter {
  ...
  func startLoading(pack: TileRegionPack) {
    ...
    var lastProgress: TileRegionLoadProgress?
  ...
  func convertRegionsToJSON(regions: [TileRegion], resolve: @escaping RCTPromiseResolveBlock, rejecter: @escaping RCTPromiseRejectBlock) {
    ...
    taskGroup.notify(queue: .main) {
      let firstError = geomteryResults.first { (_, result_and_region) in
      ...
      resolve(results.map { (_, geometry_region_metadata) -> [String: Any] in

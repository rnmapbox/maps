...
public class RNMBXShapeSource : RNMBXSource {
      ...
      parseJSON(url) { [weak self] result in
        ...
        switch result {
          ...
          case .failure(let error):
            Logger.log(level: .error, message: "[RNMBXShapeSource] Update url failed \(error) \(error.localizedDescription)")
        }
      }
    }
  }
    ...
    didSet {
      ...
      case .FeatureCollection, .Feature, .Geometry:
        do {
          ...
          Logger.log(level: .error, message: "[RNMBXShapeSource] Cannot get geoJSON object from shape \(error.localizedDescription)")
        }
      case .LineString:
        timer?.invalidate()
        ...
        do {
          ...
          Logger.log(level: .error, message: "[RNMBXShapeSource] Cannot set line object from shape \(error.localizedDescription)")
        }
      case .Point:
        do {
          ...
          Logger.log(level: .error, message: "[RNMBXShapeSource] Cannot set point from shape \(error.localizedDescription)")
        }
      default:
        break
      }
  ...
  func applyGeometryFromLine(_ line: LineString) {
    ...
    guard let lineTrimmed = line.trimmed(
      ...
      Logger.error("[RNMBXShapeSource] Line could not be trimmed")
      return
    }
  ...
  func animateToNewLineStartOffset(prevOffset: Double, targetOffset: Double?) {
    ...
    guard let duration = animationDuration?.doubleValue, duration > 0 else {
      ...
      do {
        ...
        Logger.log(level: .warn, message: "[RNMBXShapeSource] Error setting line")
      }
      return
    }
  ...
  func animateToNewLineEndOffset(prevOffset: Double, targetOffset: Double?) {
      print("[RNMBXShapeSource] animateToNewLineEndOffset is not implemented")
  }
  ...
  override func makeSource() -> Source
  {
    ...
    if let shape = shape {
      do {
        ...
        Logger.log(level: .error, message: "[RNMBXShapeSource] Unable to read shape: \(shape); Error: \(error); setting it to empty")

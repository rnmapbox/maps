...
class RNMBXModelsEventEmitter : public facebook::react::ViewEventEmitter {
 public:
...
- (void)prepareView
{
  ...
  [_view setOnStatusChanged:^(NSDictionary* event) {
    ...
    if (strongSelf != nullptr && strongSelf->_eventEmitter != nullptr) {
      ...
      ::RNMBXModelsEventEmitter::OnStatusChanged event = {type, payload};
      std::reinterpret_pointer_cast<const ::RNMBXModelsEventEmitter>(strongSelf->_eventEmitter)->onStatusChanged(event);
    }
  }];
}

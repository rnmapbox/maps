...
public class MBTilesSource: MBTileProvider {
    ...
    public static func readAsset(name: String) throws -> String {
        ...
        if bundlePath == nil {
            let assetName = name.replacingOccurrences(of: "assets_", with: "")
        ...
        let destinationPath = "\(documentsDirectory)/\(outputFileName)"

        // Remove existing file if it exists (to allow updates)

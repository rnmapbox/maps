...
const QueryTerrainElevation = ({...props}) => {
  ...
  function startAnimation(animatedRoute) {
    ...
    updateAltitudeInterval.current = setInterval(async () => {
      setAltitude(Math.floor(await map.current.queryTerrainElevation(actPoint.__getValue())))
    }, 2000);
  }
      ...
      >
        ...
        }
        {
          ...
          <Animated.ShapeSource
            ...
            <Animated.LineLayer id={'animated-route'} style={{
        ...
        {actPoint && altitude && <AnimatedMarkerView coordinate={actPoint} anchor={{x: 0.5, y: 1}}>
          <View style={{alignItems: 'center'}}>
            <View style={{backgroundColor: 'white', padding: 10, width: 140, height: 50, borderRadius: 10}}>
              <Text>Altitude: {altitude} m</Text>
            </View>
            <View style={[Styles.triangleStyle(12,'white'), {marginTop:-1}]}>
            </View>
          </View>
        </AnimatedMarkerView>}
      </MapView>

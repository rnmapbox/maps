...
public class RCTMGLStyleValue {
    ...
    public Expression getIntExpression(String key) {
        return Expression.literal(mPayload.getInt(key));
    }
    ...
    */
    public List<Double> getFloatArray(String key) {
        ReadableArray arr = getArray(key);

        ArrayList<Double> result = new ArrayList<Double>(arr.size());
        for (int i = 0; i < arr.size(); i++) {
            ReadableMap item = arr.getMap(i);
...
/*
    ...
    public Expression getFloatArrayExpression(String key) {
        return new Expression(this.getFloatArray(key));
    }
    ...
    public StyleTransition getTransition() {
        ...
        return new StyleTransition.Builder().duration(duration).delay(delay).build();
    }
}
